...
{
    ALOGD("%s: cur=%d", __func__, _ril_is_connected(ril->client));
...
int ril_set_call_volume(struct ril_handle *ril, enum ril_sound_type sound_type,
                        float volume)
{
    ALOGD("%s: type=%d vol=%d", __func__, sound_type, (int)(volume * ril->volume_steps_max));
